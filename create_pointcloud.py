...
def main(config):
    ...
    data_loader = DataLoader(
        DS_Wrapper(config.initialize("data_set", module_data), start=start, end=end),
        batch_size=1,
        shuffle=False,
        num_workers=8,
    )
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(tqdm(data_loader)):
            ...
            mask = (result["cv_mask"] >= 0.1).to(dtype=torch.float32)
            mask = (
                F.conv2d(
                    mask,
                    mask.new_ones((1, 1, mask_fill + 1, mask_fill + 1)),
                    padding=mask_fill // 2,
                )
                < 1
            ...
            if len(pose_buffer) >= buffer_length:
                ...
                mask = (
                    torch.sum(torch.stack(mask_buffer), dim=0)
                    > buffer_length - min_hits
                ).to(dtype=torch.float32)
                if use_mask:
                    depth *= mask
